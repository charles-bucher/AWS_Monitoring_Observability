terraform {
  ...
  # Remote state backend (optional, safe default)
  backend "s3" {
...
############################
locals {
  bucket_name_clean = replace(lower(var.bucket_name), "/[^a-z0-9-]/", "-")
...
module "s3_static_site" {
  ...
  bucket_name     = local.bucket_name_clean
...
module "cloudfront" {
  ...
  depends_on = [module.acm_cert, module.s3_static_site]
